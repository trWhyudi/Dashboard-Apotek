...
export const generateReport = errorHandleMiddleware(async (req, res, next) => {
  try {
    ...
    if (!allowedTypes.includes(type)) {
      return next(
        new ErrorHandler(
          "Tipe laporan tidak valid. Pilih: daily, monthly, atau yearly",
          400
        )
      );
    }
...
// Download PDF report
export const downloadReportPDF = errorHandleMiddleware(
  async (req, res, next) => {
    try {
      const { id } = req.params;
      ...
      if (!fs.existsSync(fullPath)) {
        return next(
          new ErrorHandler("File PDF tidak ditemukan di server", 404)
        );
      }
...
export const getDashboardStats = errorHandleMiddleware(
  async (req, res, next) => {
    try {
      ...
      let todayTotalMedicinesSold = 0;
      const todayTotalSales = todayTransactions.reduce((sum, t) => {
        if (t.items && Array.isArray(t.items)) {
          t.items.forEach((item) => {
            todayTotalMedicinesSold += item.quantity || 0;
          });
        }
        return sum + (t.totalAmount || 0);
      }, 0);

      let monthTotalMedicinesSold = 0;
      const monthTotalSales = monthTransactions.reduce((sum, t) => {
        if (t.items && Array.isArray(t.items)) {
          t.items.forEach((item) => {
            monthTotalMedicinesSold += item.quantity || 0;
          });
        }
        return sum + (t.totalAmount || 0);
      }, 0);
      ...
      const todayStats = {
        ...
        totalMedicinesSold: todayTotalMedicinesSold,
      };

      const monthStats = {
        totalSales: monthTotalSales,
        totalTransactions: monthTransactions.length,
        totalMedicinesSold: monthTotalMedicinesSold,
      };
